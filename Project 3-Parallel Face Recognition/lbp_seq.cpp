...
// Function to read and process images, and create histograms
void process_images(int*** training_set, int nrOfIds, int nrOfPhotosPerId, int num_rows, int num_cols) {
    for (int w = 1; w <= nrOfIds; w++) {
        for (int q = 1; q <= nrOfPhotosPerId; q++) {
            ...
            // Initialize enhanced image matrix to 0
            ...
            // Copy data from the original image to the enhanced image matrix
...
int main(int argc, char* argv[]) {
    ...
    // cout << "Sequence time for initial training set: " << seq_time << " ms" << endl;
    cout << "Sequence time: " << parallel_time_2 << " ms" << endl;
